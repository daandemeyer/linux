...
static struct file *
ovl_copy_file_range_resolve(struct file *file, enum copy_file_range_role role,
			    enum copy_file_range_resolve_mode mode)
{
	struct inode *inode = file_inode(file);
	struct file *realfile;

	if (mode == COPY_FILE_RANGE_RESOLVE_CACHED) {
...
static int ovl_copy_file_range_prepare_write(struct file *file, struct file *next)
{
	struct inode *inode = file_inode(file);
	int ret;
...
static void ovl_copy_file_range_finish_write(struct file *file, struct file *next,
...
static const struct copy_file_range_layer_operations ovl_copy_file_range_layer_ops = {
	.resolve	= ovl_copy_file_range_resolve,
	.prepare_write	= ovl_copy_file_range_prepare_write,
	.finish_write	= ovl_copy_file_range_finish_write,
...
const struct file_operations ovl_file_operations = {
	...
	.copy_file_range_layer_ops	= &ovl_copy_file_range_layer_ops,
