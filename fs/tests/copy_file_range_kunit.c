...
#define COPY_FILE_RANGE_TEST_MAGIC	0x63667274
#define COPY_FILE_RANGE_TEST_SIZE	4096
#define COPY_FILE_RANGE_TEST_MAX_EVENTS	8
#define COPY_FILE_RANGE_TEST_TIMEOUT_MS	5000

static void copy_file_range_test_put_inode(struct inode *inode)
{
	if (inode)
		iput(inode);
}

DEFINE_FREE(copy_file_range_test_iput, struct inode *,
	    copy_file_range_test_put_inode(_T))

enum copy_file_range_test_event {
	COPY_FILE_RANGE_TEST_PREPARE_0,
	COPY_FILE_RANGE_TEST_PREPARE_1,
	COPY_FILE_RANGE_TEST_FINISH_0,
	COPY_FILE_RANGE_TEST_FINISH_1,
};

struct copy_file_range_test_freeze {
...
struct copy_file_range_test_ctx {
	...
	unsigned int events[COPY_FILE_RANGE_TEST_MAX_EVENTS];
	ssize_t finish_ret[2];
	...
	loff_t remap_ret;
	struct copy_file_range_test_freeze *freeze;
	bool record_revalidate_write_readers;
};

struct copy_file_range_test_file {
	struct copy_file_range_test_ctx *ctx;
	struct file *next;
	struct file *revalidate_next;
	int resolve_error;
	enum copy_file_range_role resolve_error_role;
	enum copy_file_range_resolve_mode resolve_error_mode;
	int prepare_error;
	unsigned int layer;
};

static unsigned int
copy_file_range_test_write_readers(struct super_block *sb);

static unsigned int
copy_file_range_test_resolve_count(const struct copy_file_range_test_ctx *ctx)
{
...
static int
copy_file_range_test_verify_backing_area(int read_write, struct file *file,
					 const loff_t *ppos, size_t count)
{
	struct copy_file_range_test_file *test_file = file->private_data;
	struct copy_file_range_test_ctx *ctx = test_file->ctx;
	enum copy_file_range_role role;

	if (read_write == READ)
		role = COPY_FILE_RANGE_SOURCE;
	else if (read_write == WRITE)
		role = COPY_FILE_RANGE_DESTINATION;
	else
		return -EINVAL;
...
static struct file *
copy_file_range_test_resolve(struct file *file,
			     enum copy_file_range_role role,
			     enum copy_file_range_resolve_mode mode)
{
	...
	if (role == COPY_FILE_RANGE_DESTINATION &&
	    mode == COPY_FILE_RANGE_RESOLVE_CACHED &&
	...
	if (test_file->ctx->record_revalidate_write_readers &&
		...
		test_file->ctx->revalidate_file_write_readers[test_file->layer] =
			copy_file_range_test_write_readers(file_inode(file)->i_sb);
		test_file->ctx->revalidate_next_write_readers[test_file->layer] =
			copy_file_range_test_write_readers(file_inode(next)->i_sb);
	}
	return get_file(next);
}

static int copy_file_range_test_prepare(struct file *file, struct file *next)
{
	struct copy_file_range_test_file *test_file = file->private_data;
	struct copy_file_range_test_ctx *ctx = test_file->ctx;
	...
	if (ctx->nr_events < ARRAY_SIZE(ctx->events))
		ctx->events[ctx->nr_events++] =
			COPY_FILE_RANGE_TEST_PREPARE_0 + test_file->layer;
	return test_file->prepare_error;
}

static void copy_file_range_test_finish(struct file *file, struct file *next,
					loff_t pos_out, ssize_t ret)
{
	...
	if (ctx->nr_events < ARRAY_SIZE(ctx->events))
		ctx->events[ctx->nr_events++] =
			COPY_FILE_RANGE_TEST_FINISH_0 + test_file->layer;
}

static struct file *
copy_file_range_test_fail_resolve(struct file *file,
				  enum copy_file_range_role role,
				  enum copy_file_range_resolve_mode mode)
{
	struct copy_file_range_test_file *test_file = file->private_data;
...
static ssize_t copy_file_range_test_copy(struct file *file_in, loff_t pos_in,
					 struct file *file_out, loff_t pos_out,
					 size_t len, unsigned int flags)
{
	struct copy_file_range_test_file *test_file = file_out->private_data;
...
static ssize_t
copy_file_range_test_other_copy(struct file *file_in, loff_t pos_in,
				struct file *file_out, loff_t pos_out,
				size_t len, unsigned int flags)
{
	return -EOPNOTSUPP;
...
static loff_t
copy_file_range_test_remap(struct file *file_in, loff_t pos_in,
			   struct file *file_out, loff_t pos_out, loff_t len,
			   unsigned int remap_flags)
{
	struct copy_file_range_test_file *test_file = file_in->private_data;
...
static int copy_file_range_test_freezer(void *data)
{
	struct copy_file_range_test_freeze *freeze = data;
...
static loff_t
copy_file_range_test_freeze_remap(struct file *file_in, loff_t pos_in,
				  struct file *file_out, loff_t pos_out,
				  loff_t len, unsigned int remap_flags)
{
	struct copy_file_range_test_file *test_file = file_in->private_data;
	struct copy_file_range_test_ctx *ctx = test_file->ctx;
	struct copy_file_range_test_freeze *freeze = ctx->freeze;
	struct percpu_rw_semaphore *sem;
	struct task_struct *task;
	unsigned long timeout;

	ctx->remap_calls++;
	freeze->write_readers = copy_file_range_test_write_readers(freeze->sb);
	task = kthread_run(copy_file_range_test_freezer, freeze,
	...
	timeout = jiffies + msecs_to_jiffies(COPY_FILE_RANGE_TEST_TIMEOUT_MS);
...
static ssize_t
copy_file_range_test_splice_read(struct file *file, loff_t *ppos,
				 ...
				 unsigned int flags)
{
	...
	if (ctx->freeze)
		ctx->freeze->splice_write_readers =
			copy_file_range_test_write_readers(ctx->freeze->sb);
	return -ENODATA;
}

static const struct copy_file_range_layer_operations copy_file_range_test_layer_ops = {
	.resolve = copy_file_range_test_resolve,
	.prepare_write = copy_file_range_test_prepare,
	.finish_write = copy_file_range_test_finish,
};

static const struct copy_file_range_layer_operations copy_file_range_test_fail_layer_ops[] = {
	{
		.resolve = copy_file_range_test_fail_resolve,
		.prepare_write = copy_file_range_test_prepare,
		.finish_write = copy_file_range_test_finish,
	},
	{
...
static const struct file_operations copy_file_range_test_wrapper_fops = {
	.copy_file_range_layer_ops = &copy_file_range_test_layer_ops,
};

static const struct file_operations copy_file_range_test_method_wrapper_fops[] = {
	{
		.copy_file_range = copy_file_range_test_copy,
		.copy_file_range_layer_ops = &copy_file_range_test_layer_ops,
	},
	{
		.copy_file_range = copy_file_range_test_other_copy,
		.copy_file_range_layer_ops = &copy_file_range_test_layer_ops,
	},
	{
		.copy_file_range_layer_ops = &copy_file_range_test_layer_ops,
...
static const struct file_operations copy_file_range_test_exact_method_fops = {
	.copy_file_range = copy_file_range_test_copy,
	.copy_file_range_layer_ops = &copy_file_range_test_fail_layer_ops[0],
};

static const struct file_operations copy_file_range_test_terminal_fops[] = {
	{
		.copy_file_range = copy_file_range_test_copy,
		.copy_file_range_layer_ops = &copy_file_range_test_fail_layer_ops[0],
	},
	{
		.copy_file_range = copy_file_range_test_copy,
		.copy_file_range_layer_ops = &copy_file_range_test_fail_layer_ops[1],
	},
};

static const struct file_operations copy_file_range_test_splice_fops = {
	.fop_flags = FOP_COPY_FILE_RANGE_BACKING,
};

static const struct file_operations copy_file_range_test_remap_fops = {
	.llseek = noop_llseek,
	.splice_read = copy_file_range_test_splice_read,
	.remap_file_range = copy_file_range_test_remap,
	.fop_flags = FOP_COPY_FILE_RANGE_BACKING,
};

static const struct file_operations copy_file_range_test_freeze_fops = {
	.remap_file_range = copy_file_range_test_freeze_remap,
	.fop_flags = FOP_COPY_FILE_RANGE_BACKING,
};

static const struct file_operations copy_file_range_test_flagged_copy_fops = {
	.copy_file_range = copy_file_range_test_copy,
	.fop_flags = FOP_COPY_FILE_RANGE_BACKING,
};

static const struct file_operations copy_file_range_test_unflagged_fops = {
};

static int copy_file_range_test_init_fs_context(struct fs_context *fc)
{
	return init_pseudo(fc, COPY_FILE_RANGE_TEST_MAGIC) ? 0 : -ENOMEM;
}

static struct file_system_type copy_file_range_test_fs_types[] = {
	{
		.name = "copy_file_range_test_0",
		.init_fs_context = copy_file_range_test_init_fs_context,
		.kill_sb = kill_anon_super,
	},
	{
		.name = "copy_file_range_test_1",
		.init_fs_context = copy_file_range_test_init_fs_context,
		.kill_sb = kill_anon_super,
	},
	{
		.name = "copy_file_range_test_2",
		.init_fs_context = copy_file_range_test_init_fs_context,
		.kill_sb = kill_anon_super,
	},
};

static void copy_file_range_test_unregister_fs(void *data)
{
	unregister_filesystem(data);
}

static void copy_file_range_test_unmount(void *data)
{
	kern_unmount(data);
}

static void copy_file_range_test_fput(void *data)
{
	__fput_sync(data);
}

static void copy_file_range_test_clear_swapfile(void *data)
{
...
static struct file *
copy_file_range_test_manage_file(struct kunit *test, struct file *file,
				 struct copy_file_range_test_file *test_file)
{
	int ret;

	file->private_data = test_file;
	ret = kunit_add_action_or_reset(test, copy_file_range_test_fput, file);
	return ret ? ERR_PTR(ret) : file;
}

static struct vfsmount *
copy_file_range_test_mount(struct kunit *test, struct file_system_type *fs_type,
			   unsigned int stack_depth)
{
	...
	ret = kunit_add_action_or_reset(test, copy_file_range_test_unmount, mnt);
	if (ret)
		return ERR_PTR(ret);
	return mnt;
}
...
{
	struct inode *inode __free(copy_file_range_test_iput) =
	...
	i_size_write(inode, COPY_FILE_RANGE_TEST_SIZE);
	...
	return copy_file_range_test_manage_file(test, file, test_file);
}
...
static void
copy_file_range_test_init_mounts(struct kunit *test,
				 struct vfsmount *mounts[3])
{
	unsigned int i;
	int ret;

	for (i = 0; i < ARRAY_SIZE(copy_file_range_test_fs_types); i++) {
		struct file_system_type *fs_type =
			&copy_file_range_test_fs_types[i];

		ret = register_filesystem(fs_type);
		KUNIT_ASSERT_EQ(test, ret, 0);
		ret = kunit_add_action_or_reset(test,
						copy_file_range_test_unregister_fs,
						fs_type);
		KUNIT_ASSERT_EQ(test, ret, 0);
	}
	...
	for (i = 0; i < ARRAY_SIZE(copy_file_range_test_fs_types); i++) {
		struct file_system_type *fs_type =
			&copy_file_range_test_fs_types[i];
		unsigned int stack_depth =
			ARRAY_SIZE(copy_file_range_test_fs_types) - i - 1;

		mounts[i] = copy_file_range_test_mount(test, fs_type, stack_depth);
		KUNIT_ASSERT_NOT_ERR_OR_NULL(test, mounts[i]);
	}
	for (i = 1; i < ARRAY_SIZE(copy_file_range_test_fs_types); i++)
		KUNIT_ASSERT_PTR_NE(test, mounts[i - 1]->mnt_sb,
				    mounts[i]->mnt_sb);
}

struct copy_file_range_test_route {
	struct copy_file_range_test_ctx ctx;
	struct copy_file_range_test_file test_files[2][3];
	struct vfsmount *mounts[3];
	struct file *files[2][3];
};

static struct copy_file_range_test_route *
	...
	for (side = 0; side < ARRAY_SIZE(route->files); side++) {
		for (layer = 0; layer < ARRAY_SIZE(route->files[side]); layer++) {
			...
			fops = layer + 1 == ARRAY_SIZE(route->files[side]) ?
			       &copy_file_range_test_terminal_fops[side] :
			       &copy_file_range_test_wrapper_fops;
			user_file = layer ? route->files[side][0] : NULL;
			if (layer + 1 == ARRAY_SIZE(route->files[side]))
				cred = side == COPY_FILE_RANGE_SOURCE ?
...
static struct copy_file_range_test_route *
copy_file_range_test_alloc_paired_route(struct kunit *test)
{
...
static void
copy_file_range_test_get_refs(const struct copy_file_range_test_route *route,
...
static void
copy_file_range_test_expect_refs(struct kunit *test,
				 const struct copy_file_range_test_route *route,
...
struct copy_file_range_test_layered_file {
	struct copy_file_range_test_file wrapper_private;
	struct copy_file_range_test_file backing_private;
	struct file *wrapper;
	struct file *backing;
};

static struct copy_file_range_test_layered_file *
alloc_layered_file(struct kunit *test, struct copy_file_range_test_ctx *ctx,
...
{
	struct copy_file_range_test_layered_file *file;
...
struct copy_file_range_test_backing_route {
	struct copy_file_range_test_layered_file *source;
	struct copy_file_range_test_file destination_private;
	struct file *destination;
};

static struct copy_file_range_test_backing_route *
alloc_backing_route(struct kunit *test, struct copy_file_range_test_ctx *ctx,
		    struct vfsmount *wrapper_mnt,
		    struct vfsmount *backing_mnt,
		    const struct file_operations *backing_fops)
{
	struct copy_file_range_test_backing_route *route;
...
static void copy_file_range_test_paired_dispatch(struct kunit *test)
{
	struct copy_file_range_test_route *route;
	unsigned long refs[2][2];
	ssize_t ret;
	...
	route = copy_file_range_test_alloc_paired_route(test);
	copy_file_range_test_get_refs(route, refs);
	...
	ret = vfs_copy_file_range(route->files[COPY_FILE_RANGE_SOURCE][0], 0,
				  route->files[COPY_FILE_RANGE_DESTINATION][0],
				  0, 512, 0);
	KUNIT_EXPECT_EQ(test, ret, (ssize_t)512);
	for (layer = 0; layer < 2; layer++) {
		unsigned int (*calls)[2] = route->ctx.resolve_calls[layer];

		KUNIT_EXPECT_EQ(test, calls[COPY_FILE_RANGE_SOURCE]
					 [COPY_FILE_RANGE_RESOLVE_MAY_OPEN], 1U);
		KUNIT_EXPECT_EQ(test, calls[COPY_FILE_RANGE_DESTINATION]
					 [COPY_FILE_RANGE_RESOLVE_MAY_OPEN], 1U);
		KUNIT_EXPECT_EQ(test, calls[COPY_FILE_RANGE_SOURCE]
					 [COPY_FILE_RANGE_RESOLVE_CACHED], 0U);
		KUNIT_EXPECT_EQ(test, calls[COPY_FILE_RANGE_DESTINATION]
					 [COPY_FILE_RANGE_RESOLVE_CACHED], 1U);
		KUNIT_EXPECT_EQ(test,
	...
	copy_file_range_test_expect_refs(test, route, refs);
}

static void copy_file_range_test_credential_domains(struct kunit *test)
{
	...
	kunit_activate_static_stub(test, copy_file_range_verify_backing_area,
				   copy_file_range_test_verify_backing_area);
	ret = vfs_copy_file_range(route->files[COPY_FILE_RANGE_SOURCE][0], 0,
				  route->files[COPY_FILE_RANGE_DESTINATION][0],
				  0, 512, 0);
	...
	observed = route->ctx.resolve_creds[0][COPY_FILE_RANGE_SOURCE]
					  [COPY_FILE_RANGE_RESOLVE_MAY_OPEN];
	KUNIT_EXPECT_PTR_EQ(test, observed, caller_cred);
	observed = route->ctx.resolve_creds[0][COPY_FILE_RANGE_DESTINATION]
					  [COPY_FILE_RANGE_RESOLVE_MAY_OPEN];
	KUNIT_EXPECT_PTR_EQ(test, observed, caller_cred);
	observed = route->ctx.resolve_creds[0][COPY_FILE_RANGE_DESTINATION]
	...
	observed = route->ctx.resolve_creds[1][COPY_FILE_RANGE_SOURCE]
					  [COPY_FILE_RANGE_RESOLVE_MAY_OPEN];
	KUNIT_EXPECT_PTR_EQ(test, observed, source_cred);
	observed = route->ctx.resolve_creds[1][COPY_FILE_RANGE_DESTINATION]
					  [COPY_FILE_RANGE_RESOLVE_MAY_OPEN];
...
static void copy_file_range_test_complete_chain_alias(struct kunit *test)
{
	struct copy_file_range_test_file *alias_private;
	struct copy_file_range_test_route *route;
	...
	route = copy_file_range_test_alloc_paired_route(test);
	source = route->files[COPY_FILE_RANGE_SOURCE][0];
	destination = route->files[COPY_FILE_RANGE_DESTINATION][0];
	shared = route->files[COPY_FILE_RANGE_DESTINATION][1];
	...
	alias_private->next = route->files[COPY_FILE_RANGE_SOURCE][2];
	...
	KUNIT_ASSERT_PTR_NE(test,
			    file_inode(route->files[COPY_FILE_RANGE_SOURCE][2]),
			    file_inode(route->files[COPY_FILE_RANGE_DESTINATION][2]));

	route->test_files[COPY_FILE_RANGE_SOURCE][0].next = alias;
	copy_file_range_test_get_refs(route, refs);
...
static void copy_file_range_test_backing_terminal_selection(struct kunit *test)
{
	struct copy_file_range_test_backing_route *splice_route;
	struct copy_file_range_test_backing_route *unflagged_route;
	struct copy_file_range_test_backing_route *opaque_route;
	struct copy_file_range_test_ctx *ctx;
	struct vfsmount *mounts[3];
	ssize_t ret;
	...
	copy_file_range_test_init_mounts(test, mounts);

	splice_route =
		alloc_backing_route(test, ctx, mounts[1], mounts[2],
				    &copy_file_range_test_splice_fops);
	...
	unflagged_route =
		alloc_backing_route(test, ctx, mounts[1], mounts[2],
				    &copy_file_range_test_unflagged_fops);
	...
	opaque_route =
		alloc_backing_route(test, ctx, mounts[1], mounts[2],
				    &copy_file_range_test_flagged_copy_fops);
...
static void copy_file_range_test_hidden_swapfile(struct kunit *test)
{
	struct copy_file_range_test_backing_route *route;
	struct copy_file_range_test_ctx *ctx;
	struct vfsmount *mounts[3];
	...
	route =
		alloc_backing_route(test, ctx, mounts[1], mounts[2],
				    &copy_file_range_test_remap_fops);
	ctx->remap_ret = 512;
	...
	err = kunit_add_action_or_reset(test,
					copy_file_range_test_clear_swapfile, inode);
	...
	KUNIT_EXPECT_EQ(test, ret, (ssize_t)-EXDEV);
	KUNIT_EXPECT_EQ(test,
			ctx->resolve_calls[0][COPY_FILE_RANGE_SOURCE]
					  [COPY_FILE_RANGE_RESOLVE_CACHED], 1U);
	KUNIT_EXPECT_EQ(test,
			ctx->resolve_calls[0][COPY_FILE_RANGE_SOURCE]
					  [COPY_FILE_RANGE_RESOLVE_MAY_OPEN], 1U);
...
static void copy_file_range_test_layer_method_selection(struct kunit *test)
{
	struct copy_file_range_test_layered_file *copy_method;
	struct copy_file_range_test_layered_file *other_method;
	struct copy_file_range_test_layered_file *no_method;
	struct copy_file_range_test_ctx *ctx;
	struct vfsmount *mounts[3];
	unsigned int count;
	ssize_t ret;
	...
	copy_method =
		alloc_layered_file(test, ctx, mounts[1], mounts[2],
				   &copy_file_range_test_method_wrapper_fops[0],
				   &copy_file_range_test_unflagged_fops);
	other_method =
		alloc_layered_file(test, ctx, mounts[1], mounts[2],
				   &copy_file_range_test_method_wrapper_fops[1],
				   &copy_file_range_test_unflagged_fops);
	no_method =
		alloc_layered_file(test, ctx, mounts[1], mounts[2],
				   &copy_file_range_test_method_wrapper_fops[2],
				   &copy_file_range_test_unflagged_fops);
	...
	count = copy_file_range_test_resolve_count(ctx);
	ret = vfs_copy_file_range(copy_method->wrapper, 0,
				  no_method->wrapper, 0, 512, 0);
	KUNIT_EXPECT_EQ(test, ret, (ssize_t)-EINVAL);
	KUNIT_EXPECT_EQ(test, copy_file_range_test_resolve_count(ctx), count);
}

static void copy_file_range_test_paired_splice_route(struct kunit *test)
{
	struct copy_file_range_test_layered_file *source;
	struct copy_file_range_test_layered_file *destination;
	struct copy_file_range_test_ctx *ctx;
	struct vfsmount *mounts[3];
	unsigned int count;
	ssize_t ret;
...
static void copy_file_range_test_exact_method_precedence(struct kunit *test)
{
	struct copy_file_range_test_file *source_private;
	struct copy_file_range_test_file *destination_private;
	struct copy_file_range_test_ctx *ctx;
	struct vfsmount *mounts[3];
	...
	source = alloc_test_file(test, mounts[0],
				 &copy_file_range_test_exact_method_fops,
				 source_private, NULL);
...
static void copy_file_range_test_resolution_failure(struct kunit *test)
{
	struct copy_file_range_test_route *route;
	struct copy_file_range_test_file *fail_file;
	unsigned long refs[2][2];
	ssize_t ret;
	...
	fail_file = &route->test_files[COPY_FILE_RANGE_DESTINATION][1];
	fail_file->resolve_error = -EUCLEAN;
	fail_file->resolve_error_role = COPY_FILE_RANGE_DESTINATION;
	fail_file->resolve_error_mode = COPY_FILE_RANGE_RESOLVE_MAY_OPEN;
...
static void copy_file_range_test_revalidation_failure(struct kunit *test)
{
	struct copy_file_range_test_route *route;
	struct copy_file_range_test_file *alternate_file;
	...
	alternate_fops =
		&copy_file_range_test_terminal_fops[COPY_FILE_RANGE_DESTINATION];
	alternate = alloc_test_file(test, route->mounts[2], alternate_fops,
				    alternate_file,
				    route->files[COPY_FILE_RANGE_DESTINATION][0]);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, alternate);

	route->test_files[COPY_FILE_RANGE_DESTINATION][1].revalidate_next =
		alternate;
	copy_file_range_test_get_refs(route, refs);
	...
	KUNIT_EXPECT_EQ(test, route->ctx.finish_calls, 0U);
	KUNIT_EXPECT_EQ(test, route->ctx.copy_calls, 0U);
	cached_calls = route->ctx.resolve_calls[1][COPY_FILE_RANGE_DESTINATION]
					     [COPY_FILE_RANGE_RESOLVE_CACHED];
...
static void copy_file_range_test_nested_prepare_failure(struct kunit *test)
{
	...
	route = copy_file_range_test_alloc_paired_route(test);
	route->test_files[COPY_FILE_RANGE_DESTINATION][1].prepare_error =
		-EUCLEAN;
	copy_file_range_test_get_refs(route, refs);
...
static void copy_file_range_test_terminal_remap_results(struct kunit *test)
{
	struct copy_file_range_test_ctx *ctx;
	struct copy_file_range_test_file *test_files;
	struct vfsmount *mounts[3];
...
static void
copy_file_range_test_splice_without_terminal_freeze(struct kunit *test)
{
	struct copy_file_range_test_backing_route *route;
	struct copy_file_range_test_freeze *freeze;
	struct copy_file_range_test_ctx *ctx;
...
static void copy_file_range_test_recursive_freeze(struct kunit *test)
{
	struct copy_file_range_test_backing_route *route;
	struct copy_file_range_test_freeze *freeze;
	struct copy_file_range_test_ctx *ctx;
	...
	ctx->freeze = freeze;
	route = alloc_backing_route(test, ctx, mounts[1], mounts[2],
				    &copy_file_range_test_freeze_fops);
	freeze->sb = file_inode(route->destination)->i_sb;
	...
	completed = wait_for_completion_timeout(&freeze->done,
						msecs_to_jiffies(COPY_FILE_RANGE_TEST_TIMEOUT_MS));
	...
	if (!freeze->ret)
		thaw_ret = thaw_super(freeze->sb,
				      ...
				      freeze);
	readers = copy_file_range_test_write_readers(freeze->sb);
...
static struct kunit_case copy_file_range_test_cases[] = {
	KUNIT_CASE(copy_file_range_test_paired_dispatch),
	KUNIT_CASE(copy_file_range_test_credential_domains),
	KUNIT_CASE(copy_file_range_test_complete_chain_alias),
	KUNIT_CASE(copy_file_range_test_backing_terminal_selection),
	KUNIT_CASE(copy_file_range_test_hidden_swapfile),
	KUNIT_CASE(copy_file_range_test_layer_method_selection),
	KUNIT_CASE(copy_file_range_test_paired_splice_route),
	KUNIT_CASE(copy_file_range_test_exact_method_precedence),
	KUNIT_CASE(copy_file_range_test_resolution_failure),
	KUNIT_CASE(copy_file_range_test_revalidation_failure),
	KUNIT_CASE(copy_file_range_test_nested_prepare_failure),
	KUNIT_CASE(copy_file_range_test_terminal_remap_results),
	KUNIT_CASE(copy_file_range_test_splice_without_terminal_freeze),
	KUNIT_CASE(copy_file_range_test_recursive_freeze),
	{},
};

static struct kunit_suite copy_file_range_test_suite = {
	.name = "copy_file_range",
	.test_cases = copy_file_range_test_cases,
};

kunit_test_suite(copy_file_range_test_suite);

MODULE_DESCRIPTION("KUnit tests for copy_file_range routing");
MODULE_LICENSE("GPL");
