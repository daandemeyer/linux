...
static void cleanup_fixture(struct _test_data_copy_file_range *self)
{
...
static void cleanup_fixture_owner(struct _test_data_copy_file_range *self)
{
	if (self)
		cleanup_fixture(self);
}

DEFINE_FREE(copy_file_range_fixture, struct _test_data_copy_file_range *,
	    cleanup_fixture_owner(_T))

static int setup_fixture(struct _test_data_copy_file_range *self)
{
	...
	char plain[PATH_MAX];
	int ret;
	struct _test_data_copy_file_range *fixture
		__free(copy_file_range_fixture) = self;

	strcpy(self->root, "/tmp/copy_file_range.XXXXXX");
...
static int probe_backing_copy_file_range(struct _test_data_copy_file_range *self)
{
...
FIXTURE_SETUP(copy_file_range)
{
	...
	if (ret == -ENODEV || ret == -EOPNOTSUPP || ret == -EINVAL ||
		...
		SKIP(return, "tmpfs or overlayfs is unavailable: %s",
		     strerror(-ret));
	ASSERT_EQ(0, ret)
		TH_LOG("set up copy_file_range fixture: %s", strerror(-ret));
...
FIXTURE_TEARDOWN(copy_file_range)
{
	cleanup_fixture(self);
}
...
TEST_F(copy_file_range, translated_source_accounting)
{
...
TEST_F(copy_file_range, translated_destination_accounting)
{
	char source[PATH_MAX], destination[PATH_MAX];
...
TEST_F(copy_file_range, paired_overlay_accounting)
{
	char source[PATH_MAX], destination[PATH_MAX];
...
TEST_F_TIMEOUT(copy_file_range, translated_destination_permission_once, 30)
{
...
TEST_F_TIMEOUT(copy_file_range,
...
TEST_F_TIMEOUT(copy_file_range, permission_copyup_race, 60)
{
