...
The VFS calls ``copy_file_range_layer_ops->resolve`` without a write freeze or
inode lock held.  ``COPY_FILE_RANGE_RESOLVE_CACHED`` may not do I/O, open a
file, or mutate state; ``COPY_FILE_RANGE_RESOLVE_MAY_OPEN`` may establish
...
prepare or the terminal copy fails.  It receives the original output position
and the result from the remainder of the destination chain.  Source layers are
never prepared or write-frozen by this interface.  The optional
